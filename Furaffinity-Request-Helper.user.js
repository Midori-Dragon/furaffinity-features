...
(function () {
    ...
    class WaitAndCallAction {
        ...
        static async callFunctionAsync(functionToCall, params, action, delay, usePercent = false) {
            if (action == null) {
    ...
    class Scraps {
        ...
        async getSubmissionPageNo(username, submissionId, fromPageNumber, toPageNumber, action, delay = 100) {
    ...
    class Favorites {
        ...
        async getSubmissionDataFavId(username, submissionId, fromDataFavId, toDataFavId, maxPageNo, action, delay = 100) {
            submissionId = convertToNumber(submissionId);
        ...
        async getFiguresBetweenIds(username, fromId, toId, maxPageNo, action, delay = 100) {
            fromId = convertToNumber(fromId);
            toId = convertToNumber(toId);
            maxPageNo = convertToNumber(maxPageNo);
            if (fromId == null || fromId <= 0) {
        ...
        async getFiguresBetweenIdsBetweenPages(username, fromId, toId, fromDataFavId, toDataFavId, maxPageNo, action, delay = 100) {
        ...
        async getFiguresBetweenIdsBetweenDataIds(username, fromId, toId, fromDataFavId, toDataFavId, maxPageNo, action, delay = 100) {
            fromId = convertToNumber(fromId);
            toId = convertToNumber(toId);
            fromDataFavId = convertToNumber(fromDataFavId);
            toDataFavId = convertToNumber(toDataFavId);
        ...
        async getFiguresBetweenPages(username, fromDataFavId, toDataFavId, maxPageNo, action, delay = 100) {
            fromDataFavId = convertToNumber(fromDataFavId);
            toDataFavId = convertToNumber(toDataFavId);
            maxPageNo = convertToNumber(maxPageNo);
        ...
        async getFigures(username, fromDataFavId, direction, action, delay = 100) {
            fromDataFavId = convertToNumber(fromDataFavId);
        ...
        async getPage(username, fromDataFavId, direction, action, delay = 100) {
            fromDataFavId = convertToNumber(fromDataFavId);
            direction = convertToNumber(direction);
    ...
    async function getSubmissionDataFavId(username, submissionId, fromDataFavId, toDataFavId, maxPageNo, semaphore) {
    ...
    async function getFavoritesFiguresTillId(username, toId, fromDataFavId, maxPageNo, semaphore) {
        if (toId == null || toId <= 0) {
    ...
    async function getFavoritesFiguresSinceId(username, fromId, toDataFavId, maxPageNo, semaphore) {
        if (fromId == null || fromId <= 0) {
    ...
    async function getFavoritesFiguresBetweenIds(username, fromId, toId, fromDataFavId, toDataFavId, maxPageNo, semaphore) {
        if (fromId == null || fromId <= 0) {
            Logger.logError('No fromId given');
            return [];
        }
    ...
    async function getFavoritesFiguresTillPage(username, toDataFavId, maxPageNo, semaphore) {
        if (toDataFavId == null || toDataFavId <= 0) {
            Logger.logWarning('toDataFavId must be greater than 0. Using default 1 instead.');
            toDataFavId = -1;
        }
    ...
    async function getFavoritesFiguresSincePage(username, fromDataFavId, maxPageNo, semaphore) {
        if (fromDataFavId == null || fromDataFavId <= 0) {
            Logger.logWarning('fromDataFavId must be greater than 0. Using default 1 instead.');
            fromDataFavId = -1;
        }
    ...
    async function getFavoritesFiguresBetweenPages(username, fromDataFavId, toDataFavId, maxPageNo, semaphore) {
        if (fromDataFavId == null || fromDataFavId <= 0) {
            Logger.logWarning('fromDataFavId must be greater than 0. Using default 1 instead.');
            fromDataFavId = -1;
        }
    ...
    async function getFavoritesFigures(username, dataFavId, direction, semaphore) {
    ...
    class Journals {
        ...
        async getJournalPageNo(username, journalId, fromPageNumber, toPageNumber, action, delay = 100) {
    ...
    async function getJournalPageNo(username, journalId, fromPageNumber, toPageNumber, semaphore, percentId) {
    ...
    async function getJournalsSectionsTillId(username, toId, fromPage, semaphore) {
        if (toId == null || toId <= 0) {
            Logger.logError('No toId given');
            return [];
        }
    ...
    async function getJournalsSectionsSinceId(username, fromId, toPage, semaphore) {
        if (fromId == null || fromId <= 0) {
            Logger.logError('No fromId given');
            return [];
        }
    ...
    async function getJournalsSectionsBetweenIds(username, fromId, toId, fromPage, toPage, semaphore, percentId) {
        if (fromId == null || fromId <= 0) {
            Logger.logError('No fromId given');
            return [];
        }
    ...
    async function getJournalsSectionsTillPage(username, toPageNumber, semaphore, percentId) {
        if (toPageNumber == null || toPageNumber === 0) {
            Logger.logWarning('toPageNumber must be greater than 0. Using default 1 instead.');
            toPageNumber = 1;
        }
     ...
     */
    async function getJournalsSectionsSincePage(username, fromPageNumber, semaphore) {
        if (fromPageNumber == null || fromPageNumber <= 0) {
            Logger.logWarning('fromPageNumber must be greater than 0. Using default 1 instead.');
            fromPageNumber = 1;
        }
    ...
    async function getJournalsSectionsBetweenPages(username, fromPageNumber, toPageNumber, semaphore, percentId) {
        if (fromPageNumber == null || fromPageNumber <= 0) {
            Logger.logWarning('fromPageNumber must be greater than 0. Using default 1 instead.');
            fromPageNumber = 1;
        }
    ...
    async function getJournalsSections(username, pageNumber, semaphore) {
    ...
    class Page {
        ...
        static async getFavoritesPage(username, dataFavId, direction, semaphore) {
            if (username == null) {
                Logger.logError('No username given');
                return;
            }
    ...
    class Gallery {
        ...
        async getSubmissionPageNo(username, submissionId, folderId, fromPageNumber, toPageNumber, action, delay = 100) {
            submissionId = convertToNumber(submissionId);
    ...
    class GalleryRequests {
        ...
        static async getSubmissionPageNo(username, submissionId, folderId, fromPageNumber, toPageNumber, galleryType, semaphore, percentId) {
            if (submissionId == null || submissionId <= 0) {
                Logger.logError('No submissionId given');
                return -1;
            }
    ...
    async function nukeWatchMessagesLocal(semaphore) {
        const payload = [
    ...
    class NewJournalComments {
        ...
        async removeMessages(commentIds, action, delay = 100) {
    ...
    async function removeJournalCommentMessagesLocal(commentIds, semaphore) {
    ...
    async function nukeJournalCommentMessagesLocal(semaphore) {
        const payload = [
    ...
    class NewShouts {
        ...
        async removeMessages(shoutIds, action, delay = 100) {
    ...
    async function removeShoutMessagesLocal(shoutIds, semaphore) {
    ...
    async function nukeShoutMessagesLocal(semaphore) {
        const payload = [
    ...
    class NewFavorites {
        ...
        async removeMessages(submissionIds, action, delay = 100) {
    ...
    async function removeFavoriteMessagesLocal(submissionIds, semaphore) {
        if (submissionIds == null || submissionIds.length === 0) {
            Logger.logError('No submission ids to remove');
            return;
        }
    ...
    async function nukeFavoriteMessagesLocal(semaphore) {
        const payload = [
    ...
    class NewJournals {
        ...
        async removeMessages(journalIds, action, delay = 100) {
    ...
    async function removeJournalMessagesLocal(journalIds, semaphore) {
    ...
    async function nukeJournalMessagesLocal(semaphore) {
        const payload = [
    ...
    class NewMessages {
        ...
        async getMessagesPage(action, delay = 100) {
            return await WaitAndCallAction.callFunctionAsync(getMessagesPageLocal, [this._semaphore], action, delay);
        }
        async removeMessages(userIds, journalCommentIds, shoutIds, favoriteIds, journalIds, action, delay = 100) {
            userIds ??= [];
            journalCommentIds ??= [];
            shoutIds ??= [];
            favoriteIds ??= [];
    ...
    async function getMessagesPageLocal(semaphore) {
        return await FuraffinityRequests.getHTML(NewMessages.hardLink, semaphore);
    }
    async function removeMessagesLocal(userIds, journalCommentIds, shoutIds, favoriteIds, journalIds, semaphore) {
        const payload = [
